...
function get_param_addr_list(node::Kernel, addr_so_far::Vector{Symbol}, addr_list::Vector{Tuple{Vector{Symbol}, Any}})
    if node.tp == "real"
        a = copy(addr_so_far)
...
@gen function random_node_path(node::Kernel, biased=true)
    if biased
...
@gen function random_node_path_for_remove_c1(node::Kernel, path_so_far, paths_to_removable)
...
@gen function random_node_path_for_remove_c2(node::Kernel)
    if length(node.children)==0
        p_stop = 1
...
Custom proposals
"""
@gen function random_node_path(n::Kernel)::Union{Pair,Symbol}
...
@gen function regen_random_subtree_randomness(prev_trace::Gen.Trace)::Union{Pair,Symbol}
    path ~ random_node_path(prev_trace[:tree])
    new_subtree ~ kernel_prior()
    return path
end
